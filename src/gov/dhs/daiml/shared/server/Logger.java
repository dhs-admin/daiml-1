 ...
 */
package gov.dhs.daiml.shared.server;
...
//	}
	...
	public Logger(String logFilePath, String displayName) {
		...
		try {
			...
			logToConsole = new Boolean(DaimlProperties.LOG_TO_CONSOLE)
					.booleanValue();
			writer = new FileWriter(logFile, true);
			isClosed = false;
			format = new SimpleDateFormat("yyyy-MM-dd' 'HH:mm:ss.SSSZ");
			userLevel = Logger.Level.getType(DaimlProperties.LOG_LEVEL);
	...
	public void close() {
		...
		try {
			...
			DaimlProperties.log.error("Error trying to close app logger: "
					+ e.toString());
			isClosed = false;
		}
	}
	...
	public void error(String message) {
		writeMessage(Level.ERROR, message, true);
	}

	public void info(String message) {
		writeMessage(Level.INFO, message, true);
	}
